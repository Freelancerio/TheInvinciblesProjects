...
const RecentMatches = ({
  ...
  useEffect(() => {
    ...
    fetchMatches();
  }, [currentPage, size, season, paginated]);
        ...
        <>
          <ul className="match-list">
            {matches.map((match) => (
               <Link
                  key={match.matchId}
                  to={`/matchStatistics/${match.matchId}`}
                  className="no-underline w-full"
                >
               <li
                key={match.matchId}
                className="match-item flex justify-between items-center py-[15px] border-b border-[rgba(255,255,255,0.1)] cursor-pointer hover:bg-[rgba(255,255,255,0.05)]"
              >
                <div className="match-teams flex items-center gap-[15px] flex-1">
                  ...
                  {/* Score */}
                  <div className="match-score bg-[rgba(255,255,255,0.1)] py-1 px-3 rounded-[5px] font-bold min-w-[50px] text-center">
