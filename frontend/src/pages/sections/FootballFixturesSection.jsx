import { SearchIcon } from "lucide-react";
import React, { useState } from "react";
import { useNavigate } from "react-router-dom";
import {
  Avatar,
  AvatarFallback,
  AvatarImage,
} from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

export const FootballFixturesSection = () => {
  const [activeTab, setActiveTab] = useState("Home");
  const navigate = useNavigate();

  const navigationItems = [
    { label: "Home", value: "Home", route: "/dashboard" },
    { label: "Live", value: "Live", route: "/live" },
    { label: "Upcoming", value: "Upcoming", route: "/upcoming" },
    { label: "Results", value: "Results", route: "/results" },
    { label: "My Bets", value: "My Bets", route: "/my-bets" },
  ];

  const handleNavigation = (item) => {
    setActiveTab(item.value);
    navigate(item.route);
  };

  const handleProfileClick = () => {
    navigate("/profile");
  };

  return (
    <header className="translate-y-[-1rem] animate-fade-in opacity-0 [--animation-delay:0ms] items-center justify-between px-10 py-3 flex-[0_0_auto] border-b [border-bottom-style:solid] border-[#1e1e2f] flex relative self-stretch w-full">
      <div className="inline-flex items-center flex-[0_0_auto] gap-8 relative">
        <div className="inline-flex items-center gap-4 relative flex-[0_0_auto]">
          <img
            className="relative flex-[0_0_auto]"
            alt="SmartBet Logo"
            src="https://c.animaapp.com/mfbghyqbaFyqbB/img/depth-5--frame-0.svg"
          />

          <div className="inline-flex flex-col items-start relative flex-[0_0_auto]">
            <h1 className="relative self-stretch mt-[-1.00px] [font-family:'Inter',Helvetica] font-bold text-[#b0bec5] text-lg tracking-[0] leading-[23px] whitespace-nowrap">
              SmartBet
            </h1>
          </div>
        </div>

        <nav className="translate-y-[-1rem] animate-fade-in opacity-0 [--animation-delay:200ms] inline-flex items-center gap-9 relative flex-[0_0_auto]">
          {navigationItems.map((item, index) => (
            <Button
              key={item.value}
              variant="ghost"
              className={`h-auto p-0 inline-flex flex-col items-start relative flex-[0_0_auto] hover:bg-transparent transition-colors cursor-pointer ${activeTab === item.value ? "text-white" : "text-[#b0bec5]"
                }`}
              onClick={() => handleNavigation(item)}
            >
              <span className="relative self-stretch mt-[-1.00px] [font-family:'Inter',Helvetica] font-medium text-sm tracking-[0] leading-[21px] whitespace-nowrap">
                {item.label}
              </span>
            </Button>
          ))}
        </nav>
      </div>

      <div className="translate-y-[-1rem] animate-fade-in opacity-0 [--animation-delay:400ms] flex items-start justify-end flex-1 grow gap-8 relative">
        <div className="inline-flex flex-col min-w-40 max-w-64 items-start relative flex-[0_0_auto]">
          <div className="flex items-start relative flex-1 self-stretch w-full grow rounded-lg">
            <div className="relative self-stretch flex-[0_0_auto] flex items-center justify-center bg-[#fdfcff] rounded-[8px_0px_0px_8px] px-2">
              <SearchIcon className="w-4 h-4 text-[#b0bec5]" />
            </div>

            <div className="pl-2 pr-4 py-2 flex-1 grow bg-[#fdfcff] rounded-[0px_8px_8px_0px] overflow-hidden flex items-center relative self-stretch">
              <Input
                className="border-0 bg-transparent p-0 h-auto text-base [font-family:'Inter',Helvetica] font-normal text-[#b0bec5] placeholder:text-[#b0bec5] focus-visible:ring-0 focus-visible:ring-offset-0"
                placeholder="Search"
                defaultValue=""
              />
            </div>
          </div>
        </div>

        <img
          className="max-w-[480px] h-10 relative flex-[0_0_auto]"
          alt="Additional UI Elements"
          src="https://c.animaapp.com/mfbghyqbaFyqbB/img/depth-4--frame-1.svg"
        />

        <Avatar
          className="relative w-10 h-10 rounded-[20px] cursor-pointer hover:ring-2 hover:ring-[#b0bec5] transition-all"
          onClick={handleProfileClick}
        >
          <AvatarImage
            src="data:image/png;base64,"
            alt="User Avatar"
            className="object-cover"
          />
          <AvatarFallback className="bg-[#b0bec5] text-[#1e1e2f]">
            U
          </AvatarFallback>
        </Avatar>
      </div>
    </header>
  );
};